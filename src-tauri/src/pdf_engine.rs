...
fn render_footer_text_png(
    ...
    let font = font.as_ref().unwrap();
    let (r, g, b) = parse_hex_color(color).unwrap_or((0.58, 0.64, 0.72));
    let px_per_mm = RENDER_DPI as f32 / 25.4;
    let font_size = (font_size_mm * px_per_mm) as f32;
    let scaled_font = font.as_scaled(font_size);
    let mut text_width = 0.0f32;
    ...
    let tile_w = (text_width * 1.1).ceil() as u32;
    let tile_h = (font_size * 1.3).ceil() as u32;
    let mut img = image::RgbaImage::new(tile_w, tile_h);
    let text_color = [(r * 255.0) as u8, (g * 255.0) as u8, (b * 255.0) as u8, 255u8];
    let x_start = (tile_w as f32 - text_width) / 2.0;
    let y_baseline = tile_h as f32 * 0.7;
    let mut x = x_start;
    for c in text.chars() {
        let glyph_id = font.glyph_id(c);
        let glyph = ab_glyph::Glyph {
        ...
        if let Some(q) = font.outline_glyph(glyph) {
            ...
            q.draw(|gx, gy, v| {
                let px = (x_draw + gx as f32) as i32;
                let py = (y_draw + gy as f32) as i32;
                if px >= 0 && py >= 0 && (px as u32) < tile_w && (py as u32) < tile_h {
                    let pixel = img.get_pixel(px as u32, py as u32);
                    let alpha = (v * text_color[3] as f32) as u8;
                    if alpha > pixel[3] {
                        img.put_pixel(px as u32, py as u32, image::Rgba([text_color[0], text_color[1], text_color[2], alpha]));
                    }
                }
            });
        }
...
// =====================================================
// XPS Print Engine — 静默打印（不增加体积）
// =====================================================
struct XpsImageResource {
    file_idx: usize,
    img_bytes: Vec<u8>,
    img_ext: String,
}
...
pub fn xps_print(
    ...
    let total_files = request.files.len() as u32;
    if let Some(ref cb) = &on_progress {
        cb("xps-decode", 0, total_files);
    }
    let sources = decode_images(&request.files, &request.settings);
    if let Some(ref cb) = &on_progress {
        cb("xps-decode", total_files, total_files);
    }

    let total_pages = request.pages.len() as u32;
    let (xps_bytes, font_warning) = build_xps_bytes(
        request,
        &slot_positions,
        pw,
        ph,
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    let xps_path = temp_dir.join(format!("fapiao_print_{}.xps", timestamp));

    // 写入 XPS 文件
    std::fs::write(&xps_path, &xps_bytes)
        .map_err(|e| format!("写入 XPS 文件失败: {}", e))?;

    // 打印 XPS 文件
    #[cfg(target_os = "windows")]
    {
        shell_execute_print_xps_file(&xps_path, printer_name)?;
    }

    // 清理临时文件（忽略错误）
    let _ = std::fs::remove_file(&xps_path);

    Ok(font_warning)
}

#[cfg(target_os = "windows")]
fn shell_execute_print_xps_file(xps_path: &std::path::Path, printer_name: &str) -> Result<(), String> {
    use windows::core::{HSTRING, PCWSTR};
    use windows::Win32::UI::Shell::ShellExecuteW;
    use windows::Win32::UI::WindowsAndMessaging::{SW_HIDE, SW_SHOW};

    let resolved_printer: Option<String> = if printer_name.is_empty() {
    ...
    unsafe {
        ...
        let file: HSTRING = xps_path.to_string_lossy().to_string().into();
...
fn build_xps_bytes(
    ...
    let zip_opts_stored: SimpleFileOptions = SimpleFileOptions::default()
        .compression_method(zip::CompressionMethod::Stored)
        .unix_permissions(0o644);

    let mut image_resources: Vec<XpsImageResource> = Vec::new();

    for (file_idx, source_opt) in sources.iter().enumerate() {
        let Some(source) = source_opt else { continue };
        let (img_bytes, img_ext) = extract_xps_image_bytes(source)?;
        image_resources.push(XpsImageResource { file_idx, img_bytes, img_ext });
    }

    let mut content_types_parts = String::new();
    content_types_parts.push_str("<Override PartName=\"/FixedDocumentSequence.fdseq\" ContentType=\"application/vnd.ms-package.xps-fixeddocumentsequence+xml\"/>");
    content_types_parts.push_str("<Override PartName=\"/_rels/.rels\" ContentType=\"application/vnd.openxmlformats-package.relationships+xml\"/>");
    content_types_parts.push_str("<Override PartName=\"/Documents/1/FixedDocument.fdoc\" ContentType=\"application/vnd.ms-package.xps-fixeddocument+xml\"/>");
    content_types_parts.push_str("<Override PartName=\"/Documents/1/_rels/FixedDocument.fdoc.rels\" ContentType=\"application/vnd.openxmlformats-package.relationships+xml\"/>");
    content_types_parts.push_str("<Override PartName=\"/_rels/FixedDocumentSequence.fdseq.rels\" ContentType=\"application/vnd.openxmlformats-package.relationships+xml\"/>");

    for res in &image_resources {
        let img_path = format!("/Documents/1/Resources/Images/img{}.{}", res.file_idx, res.img_ext);
        let ct = if res.img_ext == "jpg" || res.img_ext == "jpeg" { "image/jpeg" } else { "image/png" };
        content_types_parts.push_str(&format!("<Override PartName=\"{}\" ContentType=\"{}\"/>", img_path, ct));
    }

    for (page_idx, _page_spec) in request.pages.iter().enumerate() {
        let page_path = format!("/Documents/1/Pages/{}.fpage", page_idx + 1);
        content_types_parts.push_str(&format!("<Override PartName=\"{}\" ContentType=\"application/vnd.ms-package.xps-fixedpage+xml\"/>", page_path));
        let rels_path = format!("/Documents/1/Pages/_rels/{}.fpage.rels", page_idx + 1);
        content_types_parts.push_str(&format!("<Override PartName=\"{}\" ContentType=\"application/vnd.openxmlformats-package.relationships+xml\"/>", rels_path));
    }

    let _ = zip.start_file("[Content_Types].xml", zip_opts);
    ...
    zip.write_all(content_types_xml.as_bytes()).map_err(|e| format!("XPS写入失败: {}", e))?;

    let _ = zip.start_file("_rels/.rels", zip_opts);
    zip.write_all(b"<?xml version=\"1.0\" encoding=\"UTF-8\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"><Relationship Id=\"R1\" Target=\"/FixedDocumentSequence.fdseq\" Type=\"http://schemas.microsoft.com/xps/2005/06/fixedrepresentation\"/></Relationships>").map_err(|e| format!("XPS写入失败: {}", e))?;

    for res in &image_resources {
        let img_path = format!("Documents/1/Resources/Images/img{}.{}", res.file_idx, res.img_ext);
        let _ = zip.start_file(&img_path, zip_opts_stored);
        zip.write_all(&res.img_bytes).map_err(|e| format!("XPS写入失败: {}", e))?;
    }

    let text_font = load_system_font();
    let needs_text = request.settings.page_num
        || request.settings.print_date
        || request.settings.footer_text.as_ref().map_or(false, |t| !t.is_empty())
        || (request.settings.watermark && request.settings.watermark_text.as_ref().map_or(false, |t| !t.is_empty()));
    let font_warning = if text_font.is_none() && needs_text {
        Some("系统缺少中文字体(simhei.ttf)，页脚/水印/页码将不显示".to_string())
    } else {
        None
    };

    let mut doc_rels_parts = String::new();
    for (page_idx, page_spec) in request.pages.iter().enumerate() {
        if SHUTTING_DOWN.load(Ordering::SeqCst) {
            return Err("应用正在关闭".to_string());
        }
        if let Some(ref cb) = &on_progress {
            cb("xps-page", (page_idx + 1) as u32, total_pages);
        }

        let (page_xml, page_rels, extra_resources) = build_xps_page_xml(
            page_idx,
            page_spec,
            request,
            slot_positions,
        ...
        for res in &extra_resources {
            let img_path = format!("Documents/1/Resources/Images/{}.{}", res.file_idx, res.img_ext);
            let _ = zip.start_file(&img_path, zip_opts_stored);
            zip.write_all(&res.img_bytes).map_err(|e| format!("XPS写入失败: {}", e))?;
        }

        let page_path = format!("Documents/1/Pages/{}.fpage", page_idx + 1);
        let _ = zip.start_file(&page_path, zip_opts);
        zip.write_all(page_xml.as_bytes()).map_err(|e| format!("XPS写入失败: {}", e))?;

        let rels_path = format!("Documents/1/Pages/_rels/{}.fpage.rels", page_idx + 1);
        let _ = zip.start_file(&rels_path, zip_opts);
        zip.write_all(page_rels.as_bytes()).map_err(|e| format!("XPS写入失败: {}", e))?;

        doc_rels_parts.push_str(&format!(
            "<Relationship Id=\"R{}\" Target=\"Pages/{}.fpage\" Type=\"http://schemas.microsoft.com/xps/2005/06/fixedpage\"/>",
            page_idx + 1, page_idx + 1
        ));
    }

    let _ = zip.start_file("Documents/1/_rels/FixedDocument.fdoc.rels", zip_opts);
    ...
    zip.write_all(doc_rels_xml.as_bytes()).map_err(|e| format!("XPS写入失败: {}", e))?;

    let _ = zip.start_file("Documents/1/FixedDocument.fdoc", zip_opts);
    let doc_xml = build_xps_doc_xml(request.pages.len());
    zip.write_all(doc_xml.as_bytes()).map_err(|e| format!("XPS写入失败: {}", e))?;

    let _ = zip.start_file("FixedDocumentSequence.fdseq", zip_opts);
    let seq_xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><FixedDocumentSequence xmlns=\"http://schemas.microsoft.com/xps/2005/06\" xml:lang=\"zh-CN\"><DocumentReference Source=\"Documents/1/FixedDocument.fdoc\"/></FixedDocumentSequence>";
    zip.write_all(seq_xml.as_bytes()).map_err(|e| format!("XPS写入失败: {}", e))?;

    let _ = zip.start_file("_rels/FixedDocumentSequence.fdseq.rels", zip_opts);
    let seq_rels_xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"><Relationship Id=\"R1\" Target=\"/Documents/1/FixedDocument.fdoc\" Type=\"http://schemas.microsoft.com/xps/2005/06/fixeddocument\"/></Relationships>";
    zip.write_all(seq_rels_xml.as_bytes()).map_err(|e| format!("XPS写入失败: {}", e))?;

    zip.finish().map_err(|e| format!("XPS ZIP写入失败: {}", e))?;
    Ok((buf, font_warning))
}

fn extract_xps_image_bytes(source: &ImageSource) -> Result<(Vec<u8>, String), String> {
    match source {
        ImageSource::JpegPassthrough { raw_bytes, .. } => {
            Ok((raw_bytes.clone(), "jpg".to_string()))
        }
        ImageSource::Decoded(img) => {
            let mut png_buf = Vec::new();
            img.write_to(&mut std::io::Cursor::new(&mut png_buf), image::ImageFormat::Png)
                .map_err(|e| format!("PNG编码失败: {}", e))?;
            Ok((png_buf, "png".to_string()))
        }
    }
}

fn build_xps_doc_xml(num_pages: usize) -> String {
    let mut pages = String::new();
    for i in 1..=num_pages {
        pages.push_str(&format!("<PageContent Source=\"Pages/{}.fpage\"/>", i));
    }
    format!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?><FixedDocument xmlns=\"http://schemas.microsoft.com/xps/2005/06\" xml:lang=\"zh-CN\">{}</FixedDocument>",
        pages
    )
}

#[cfg(target_os = "windows")]
fn build_xps_page_xml(
    page_idx: usize,
    page_spec: &PageSpec,
    request: &LayoutRenderRequest,
    slot_positions: &[LayoutSlotMm],
    ...
    ph: f32,
    sources: &[Option<ImageSource>],
    text_font: &Option<ab_glyph::FontArc>,
) -> (String, String, Vec<XpsImageResource>) {
    let pw_px = pw * 96.0 / 25.4;
    let ph_px = ph * 96.0 / 25.4;
    let s = page_idx + 1;
    let total = request.pages.len();

    let mut content = String::new();
    let mut rels_parts = String::new();
    let mut extra_resources: Vec<XpsImageResource> = Vec::new();

    let has_page_num = request.settings.page_num;
    let has_print_date = request.settings.print_date;
    let has_footer_text = request.settings.footer_text.as_ref().map_or(false, |t| !t.is_empty());

    let text_bottom_mm = 5.0;
    let mut line_idx = 0;
    let footer_margin = if has_page_num || has_print_date || has_footer_text {
        if request.settings.custom_fm {
            request.settings.footer_margin
        } else {
            let line_count = (has_page_num as u32) + (has_print_date as u32) + (has_footer_text as u32);
            3.0 + line_count as f32 * 5.0
        }
    } else {
        0.0
    };

    let has_any_footer = has_page_num || has_print_date || has_footer_text;
    let draw_area_h = if has_any_footer { ph - footer_margin } else { ph };

    let bottom_margin_y = request.settings.margin_bottom + footer_margin;

    if request.settings.cutline && (request.settings.cols > 1 || request.settings.rows > 1) {
        for c in 1..request.settings.cols as usize {
            let slot_w = (pw - request.settings.cols as f32 * (request.settings.margin_left + request.settings.margin_right) - (request.settings.cols as f32 - 1.0) * request.settings.gap_h) / request.settings.cols as f32;
            let x = request.settings.margin_left + c as f32 * (slot_w + request.settings.margin_left + request.settings.margin_right + request.settings.gap_h) - request.settings.gap_h / 2.0;
            let x_px = x / pw * pw_px;
            let y1_px = request.settings.margin_top / ph * ph_px;
            let y2_px = (ph - bottom_margin_y) / ph * ph_px;
            content.push_str(&format!(
                "<Path StrokeThickness=\"0.5\" Stroke='#CCCCCC' Data='M{:.2},{:.2} L{:.2},{:.2}'/>",
                x_px, y1_px, x_px, y2_px
            ));
        }
        for r in 1..request.settings.rows as usize {
            // 注：下面的 `y` 是 BOTTOM-UP 坐标（Y=0 在纸张底部），
            // 计算方式是：从底部 margin_bottom 往上累加 slot_h + margin + gap。
            // XPS 使用 TOP-DOWN 坐标（Y=0 在纸张顶部），
            // 因此转换为 XPS 像素坐标时需要用 (ph - y) 做翻转。
            let slot_h = (draw_area_h - request.settings.rows as f32 * (request.settings.margin_bottom + request.settings.margin_top) - (request.settings.rows as f32 - 1.0) * request.settings.gap_v) / request.settings.rows as f32;
            let y = request.settings.margin_bottom + footer_margin + r as f32 * (slot_h + request.settings.margin_bottom + request.settings.margin_top + request.settings.gap_v) - request.settings.gap_v / 2.0;
            // y 是 bottom-up 坐标，XPS 需要 top-down，所以用 (ph - y) 翻转
            let y_px = (ph - y) / ph * ph_px;
            let x1_px = request.settings.margin_left / pw * pw_px;
            let x2_px = (pw - request.settings.margin_right) / pw * pw_px;
            content.push_str(&format!(
                "<Path StrokeThickness=\"0.5\" Stroke='#CCCCCC' Data='M{:.2},{:.2} L{:.2},{:.2}'/>",
                x1_px, y_px, x2_px, y_px
            ));
        }
    }

    let per_page = request.settings.cols * request.settings.rows;
    for (slot_idx, slot) in page_spec.slots.iter().enumerate() {
        if slot_idx >= per_page as usize { break; }
        let Some(file_idx) = slot.file_index else { continue };
        let source = sources.get(file_idx).and_then(|s| s.as_ref());
        let Some(source) = source else { continue };

        let pos = &slot_positions[slot_idx];

        let img_rel_id = format!("Rimg{}", file_idx);
        let img_ext = match source {
            ImageSource::JpegPassthrough { .. } => "jpg",
            ImageSource::Decoded(_) => "png",
        };
        let img_target = format!("../../Resources/Images/img{}.{}", file_idx, img_ext);

        let img_result = prepare_xps_image(source, file_idx, slot.rotation, &request.settings, pos, slot);
        let Some((img_xml, _draw_w_px, _draw_h_px)) = img_result else { continue };

        let img_xml_with_rel = img_xml.replace(
            &format!("{{img{}}}", file_idx),
            &img_target,
        );
        content.push_str(&img_xml_with_rel);

        rels_parts.push_str(&format!(
            "<Relationship Id=\"{}\" Target=\"{}\" Type=\"http://schemas.microsoft.com/xps/2005/06/required-resource\"/>",
            img_rel_id, img_target
        ));
        ...
        if request.settings.watermark {
            ...
            if !wm_text.is_empty() {
                ...
                let wm_size = request.settings.watermark_size;
                let wm_angle = request.settings.watermark_angle;
                if let Some((png_bytes, wm_w, wm_h)) = render_watermark(&text_font, &wm_text, &wm_color, wm_opacity, wm_size, wm_angle) {
                    let wm_res_idx = 10000 + page_idx * 1000 + slot_idx;
                    extra_resources.push(XpsImageResource {
                        file_idx: wm_res_idx,
                        img_bytes: png_bytes,
                        img_ext: "png".to_string(),
                    });

                    let wm_target = format!("../../Resources/Images/{}.png", wm_res_idx);
                    let wm_rel_id = format!("Rwm{}", wm_res_idx);

                    let px_per_mm = RENDER_DPI as f32 / 25.4;
                    let wm_w_mm = wm_w as f32 / px_per_mm;
                    let wm_h_mm = wm_h as f32 / px_per_mm;
                    let wm_w_px = wm_w_mm / pw * pw_px;
                    let wm_h_px = wm_h_mm / ph * ph_px;

                    let wm_x_px = (pos.x_mm + (pos.w_mm - wm_w_mm) / 2.0) / pw * pw_px;
                    let wm_y_px = (ph - pos.y_mm - (pos.h_mm + wm_h_mm) / 2.0) / ph * ph_px;

                    content.push_str(&format!(
                        "<Path Data='M{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} Z'>\
                        <Path.Fill><ImageBrush ImageSource='{}' Stretch='Fill' ViewboxUnits='Absolute' ViewportUnits='Absolute' TileMode='None' Viewport='{:.2},{:.2},{:.2},{:.2}' Viewbox='0,0,{:.2},{:.2}'/>\
                        </Path.Fill></Path>",
                        wm_x_px, wm_y_px,
                        wm_x_px + wm_w_px, wm_y_px,
                        wm_x_px + wm_w_px, wm_y_px + wm_h_px,
                        wm_x_px, wm_y_px + wm_h_px,
                        wm_target, wm_x_px, wm_y_px, wm_w_px, wm_h_px, wm_w as f32, wm_h as f32
                    ));
        ...
        if request.settings.number {
            let start_num = page_idx * per_page as usize + 1;
            let num_images = render_slot_numbers(&text_font, &slot_positions, start_num);
            if slot_idx < num_images.len() {
                let (png_bytes, num_w, num_h) = &num_images[slot_idx];

                let num_res_idx = 20000 + page_idx * 1000 + slot_idx;
                extra_resources.push(XpsImageResource {
                    file_idx: num_res_idx,
                    img_bytes: png_bytes.clone(),
                    img_ext: "png".to_string(),
                });

                let num_target = format!("../../Resources/Images/{}.png", num_res_idx);
                let num_rel_id = format!("Rnum{}", num_res_idx);

                let px_per_mm = RENDER_DPI as f32 / 25.4;
                let num_w_mm = *num_w as f32 / px_per_mm;
                let num_h_mm = *num_h as f32 / px_per_mm;
                let num_w_px = num_w_mm / pw * pw_px;
                let num_h_px = num_h_mm / ph * ph_px;

                let padding_mm = 2.0;
                let num_x_px = (pos.x_mm + pos.w_mm - num_w_mm - padding_mm) / pw * pw_px;
                let num_y_px = (ph - pos.y_mm - pos.h_mm) / ph * ph_px;
                ...
                content.push_str(&format!(
                    ...
                    num_x_px, num_y_px,
                    num_x_px + num_w_px, num_y_px,
                    num_x_px + num_w_px, num_y_px + num_h_px,
                    num_x_px, num_y_px + num_h_px,
                    num_target, num_x_px, num_y_px, num_w_px, num_h_px, *num_w as f32, *num_h as f32
                ));

                rels_parts.push_str(&format!(
                    "<Relationship Id=\"{}\" Target=\"{}\" Type=\"http://schemas.microsoft.com/xps/2005/06/required-resource\"/>",
                    num_rel_id, num_target
                ));
            }
        }

        if request.settings.border {
            let border_color = request.settings.border_color.as_deref().unwrap_or("#000000");
            let bw = request.settings.border_width.unwrap_or(1.0);
            let bw_px = bw / pw * pw_px;
            let x1_px = pos.x_mm / pw * pw_px;
            let y1_px = (ph - pos.y_mm - pos.h_mm) / ph * ph_px;
            let x2_px = (pos.x_mm + pos.w_mm) / pw * pw_px;
            let y2_px = (ph - pos.y_mm) / ph * ph_px;
            content.push_str(&format!(
                "<Path StrokeThickness=\"{:.2}\" Stroke='{}' Data='M{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} Z'/>",
                bw_px, border_color, x1_px, y1_px, x2_px, y1_px, x2_px, y2_px, x1_px, y2_px
            ));
        }
    }

    if has_page_num || has_print_date || has_footer_text {
        let footer_font_size_mm = 3.0;
        let line_h_mm_footer = 5.0;
        let px_per_mm = RENDER_DPI as f32 / 25.4;
        let mut footer_y_mm = text_bottom_mm;

        if has_page_num {
            let text = format!("第 {} 页 / 共 {} 页", s, total);
            if let Some((png_bytes, fw, fh)) = render_footer_text_png(&text_font, &text, "#94a3b8", footer_font_size_mm) {
                let footer_res_idx = 30000 + page_idx * 1000 + line_idx;
                extra_resources.push(XpsImageResource { file_idx: footer_res_idx, img_bytes: png_bytes, img_ext: "png".to_string() });
                let footer_target = format!("../../Resources/Images/{}.png", footer_res_idx);
                let fw_mm = fw as f32 / px_per_mm;
                let fh_mm = fh as f32 / px_per_mm;
                let fw_px = fw_mm / pw * pw_px;
                let fh_px = fh_mm / ph * ph_px;
                let fx_px = (pw / 2.0 - fw_mm / 2.0) / pw * pw_px;
                let fy_px = (ph - footer_y_mm - fh_mm) / ph * ph_px;
                content.push_str(&format!(
                    "<Path Data='M{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} Z'>\
                    <Path.Fill><ImageBrush ImageSource='{}' Stretch='Fill' ViewboxUnits='Absolute' ViewportUnits='Absolute' TileMode='None' Viewport='{:.2},{:.2},{:.2},{:.2}' Viewbox='0,0,{:.2},{:.2}'/>\
                    </Path.Fill></Path>",
                    ...
                    fx_px, fy_px + fh_px,
                    footer_target, fx_px, fy_px, fw_px, fh_px, fw as f32, fh as f32
                ));
                rels_parts.push_str(&format!(
                    "<Relationship Id=\"Rft{}\" Target=\"{}\" Type=\"http://schemas.microsoft.com/xps/2005/06/required-resource\"/>",
                    footer_res_idx, footer_target
                ));
            }
            footer_y_mm += line_h_mm_footer;
            line_idx += 1;
        }
        if has_print_date {
            let date_str = get_local_date_string();
            let text = format!("打印日期 {}", date_str);
            if let Some((png_bytes, fw, fh)) = render_footer_text_png(&text_font, &text, "#94a3b8", footer_font_size_mm) {
                let footer_res_idx = 30000 + page_idx * 1000 + line_idx;
                extra_resources.push(XpsImageResource { file_idx: footer_res_idx, img_bytes: png_bytes, img_ext: "png".to_string() });
                let footer_target = format!("../../Resources/Images/{}.png", footer_res_idx);
                ...
                let fw_px = fw_mm / pw * pw_px;
                let fh_px = fh_mm / ph * ph_px;
                let fx_px = (pw - fw_mm - 5.0) / pw * pw_px;
                let fy_px = (ph - footer_y_mm - fh_mm) / ph * ph_px;
                content.push_str(&format!(
                    "<Path Data='M{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} Z'>\
                    <Path.Fill><ImageBrush ImageSource='{}' Stretch='Fill' ViewboxUnits='Absolute' ViewportUnits='Absolute' TileMode='None' Viewport='{:.2},{:.2},{:.2},{:.2}' Viewbox='0,0,{:.2},{:.2}'/>\
        ...
        if has_footer_text {
            let text = request.settings.footer_text.as_ref().unwrap();
            if let Some((png_bytes, fw, fh)) = render_footer_text_png(&text_font, text, "#94a3b8", footer_font_size_mm) {
                let footer_res_idx = 30000 + page_idx * 1000 + line_idx;
                extra_resources.push(XpsImageResource { file_idx: footer_res_idx, img_bytes: png_bytes, img_ext: "png".to_string() });
                let footer_target = format!("../../Resources/Images/{}.png", footer_res_idx);
    ...
    let page_xml = format!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?><FixedPage Width=\"{:.2}\" Height=\"{:.2}\" xmlns=\"http://schemas.microsoft.com/xps/2005/06\" xml:lang=\"zh-CN\">{}</FixedPage>",
        pw_px, ph_px, content
    );

    let page_rels = format!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\">{}</Relationships>",
        rels_parts
    );

    (page_xml, page_rels, extra_resources)
}

fn prepare_xps_image(
    source: &ImageSource,
    file_idx: usize,
    rotation: i32,
    settings: &RenderSettings,
    ...
    let ph = settings.paper_h;
    let mm_to_xps: f32 = 96.0 / 25.4;

    let (src_w, src_h) = match source {
        ImageSource::JpegPassthrough { width, height, .. } => (*width as f32, *height as f32),
        ImageSource::Decoded(img) => (img.width() as f32, img.height() as f32),
    };
    ...
    let is_rotated = rot == 90 || rot == 270;
    let natural_w = if is_rotated { src_h } else { src_w };
    let natural_h = if is_rotated { src_w } else { src_h };

    let (scale_x, scale_y) = calculate_fit_scale_xy(natural_w, natural_h, slot.w_mm, slot.h_mm, &settings.fit_mode, settings.custom_scale);

    let slot_scale = slot_spec.scale.unwrap_or(1.0);
    let final_scale_x = scale_x * slot_scale;
    let final_scale_y = scale_y * slot_scale;

    let draw_w = natural_w * final_scale_x;
    let draw_h = natural_h * final_scale_y;

    let slot_offset_x = slot_spec.offset_x.unwrap_or(0.0);
    let slot_offset_y = slot_spec.offset_y.unwrap_or(0.0);

    // Image center in XPS page coordinates (top-down, units = 1/96 inch)
    let center_x_xps = (slot.x_mm + slot.w_mm / 2.0 + slot_offset_x) * mm_to_xps;
    let center_y_xps = (ph - slot.y_mm - slot.h_mm / 2.0 + slot_offset_y) * mm_to_xps;

    let transform = build_xps_transform(rot, src_w, src_h, final_scale_x, final_scale_y, center_x_xps, center_y_xps);

    // Clip path = slot rectangle in XPS page coordinates
    let slot_w_px = slot.w_mm * mm_to_xps;
    let slot_h_px = slot.h_mm * mm_to_xps;
    let base_x_px = slot.x_mm * mm_to_xps;
    let base_y_px = (ph - slot.y_mm - slot.h_mm) * mm_to_xps;

    // ImageBrush with required Viewport and Viewbox attributes
    let img_xml = format!(
        "<Path Data='M{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} L{:.2},{:.2} Z'>\
        <Path.Fill><ImageBrush ImageSource='{{img{}}}' Stretch='Fill' \
        ViewboxUnits='Absolute' ViewportUnits='Absolute' TileMode='None' \
        Viewbox='0,0,{:.0},{:.0}' Viewport='0,0,{:.0},{:.0}'>\
        <ImageBrush.Transform><MatrixTransform Matrix='{}'/></ImageBrush.Transform>\
        </ImageBrush></Path.Fill></Path>",
        base_x_px, base_y_px,
        base_x_px + slot_w_px, base_y_px,
        base_x_px + slot_w_px, base_y_px + slot_h_px,
        base_x_px, base_y_px + slot_h_px,
        file_idx,
        src_w, src_h,
        src_w, src_h,
        transform
    ...
    let (final_w_px, final_h_px) = (
        draw_w * mm_to_xps,
        draw_h * mm_to_xps,
    );

    Some((img_xml, final_w_px, final_h_px))
}

fn calculate_fit_scale_xy(img_w: f32, img_h: f32, slot_w: f32, slot_h: f32, fit_mode: &str, custom_scale: f32) -> (f32, f32) {
    let scale_w = slot_w / img_w;
    let scale_h = slot_h / img_h;
    if custom_scale > 0.0 {
        let contain_s = scale_w.min(scale_h);
        let s = contain_s * custom_scale;
        return (s, s);
    }
    match fit_mode {
        "contain" => {
...
fn build_xps_transform(
    ...
    // Scale factors convert from source pixels to XPS page units.
    // For 0°/180°: sx maps source X to display X, sy maps source Y to display Y.
    // For 90°/270°: after rotation, source X maps to display Y and vice versa,
    //   so we swap which final_scale applies to which source axis.
    let (sx, sy) = if is_rotated {
        (final_scale_y * mm_to_xps, final_scale_x * mm_to_xps)
    } else {
        (final_scale_x * mm_to_xps, final_scale_y * mm_to_xps)
    };

    let cw = src_w / 2.0;
    let ch = src_h / 2.0;

    let angle_rad = match rotation {
        90 => std::f32::consts::FRAC_PI_2,
        180 => std::f32::consts::PI,
        270 => std::f32::consts::FRAC_PI_2 * 3.0,
        _ => 0.0,
    };
    ...
    // Combined matrix: Translate(Cx,Cy) * Rotate(θ) * Scale(sx,sy) * Translate(-cw,-ch)
    // XPS Matrix format: "M11,M12,M21,M22,OffsetX,OffsetY"
    // where: x' = M11*x + M21*y + OffsetX
    //        y' = M12*x + M22*y + OffsetY
    let m11 = cos_a * sx;
    let m12 = sin_a * sx;
    let m21 = -sin_a * sy;
    let m22 = cos_a * sy;
    let off_x = -cos_a * sx * cw + sin_a * sy * ch + center_x_xps;
    let off_y = -sin_a * sx * cw - cos_a * sy * ch + center_y_xps;

    format!("{:.6},{:.6},{:.6},{:.6},{:.2},{:.2}", m11, m12, m21, m22, off_x, off_y)
}
...
fn get_local_date_string() -> String {
    ...
    format!("{}-{:02}-{:02}", st.wYear, st.wMonth, st.wDay)
}
