...
- **单票独立调整**（v1.9.0+，v2.0.1-v2.0.2 增强）：每张发票预览拖拽移动 + 角落 handle 缩放，九宫格快速对齐，滚轮微调 + 滚轮单票缩放（5%/步），±150mm 偏移范围，拖拽约束动态化，放大上限 3x，编辑态溢出预览，调整参数可选持久化记忆，侧边栏「单票调整」面板或发票弹窗参数编辑，PDF 按参数裁剪输出
